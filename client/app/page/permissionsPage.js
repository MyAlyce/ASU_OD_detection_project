...
Page({
	...
	onInit() {
		...
		if (lastVisitTime && currentTime - parseInt(lastVisitTime) > 2000) {
			console.log(
				'DEBUG: Returning to permissions page, checking for updates from settings',
			);
			...
			if (globalPermissions) {
				try {
					...
					const lastUpdateTime = this.localStorage.getItem(
						'lastPermissionsUpdate',
					);

					// If the global permissions were updated after our last visit, use them
					if (
						!lastUpdateTime ||
						parsedPermissions.timestamp > parseInt(lastUpdateTime)
					) {
						console.log(
							'DEBUG: Found newer permissions in global storage, updating state',
						);
		...
		try {
			...
			try {
				...
				if (appSideService) {
					...
					console.log(
						`DEBUG: Sent ${toggleId} update to app-side service: ${isChecked}`,
					);
				} else {
					console.log(
						'DEBUG: No app-side service available, using alternative method',
					);
					// Alternative method: Set a special key in global storage
					this.globalStorage.setKey(
						'permissions_sync_request',
						JSON.stringify({
